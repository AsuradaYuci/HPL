...
class LTCC(object):
    ...
    """
    dataset_dir = 'LTCC'
    msk_dir = 'schp_atr_mask_merge=6/train'
    ...
    def _process_dir_train(self, dir_path, mask_dir):
        ...
        for img_path in img_paths:
            ...
            pid, _, _ = map(int, pattern1.search(img_name).groups())
            clothes_id = pattern2.search(img_name).group(1)
        ...
        for img_path in img_paths:
            img_name = osp.basename(img_path)
            pid, _, camid = map(int, pattern1.search(img_name).groups())
            clothes = pattern2.search(img_name).group(1)
            camid -= 1 # index starts from 0
            pid = pid2label[pid]
            clothes_id = clothes2label[clothes]
            name = img_name.split('.')[0] + '.npy'
            msk_path = osp.join(mask_dir, name)
            dataset.append((img_path, pid, camid, clothes_id, msk_path))
