...
class ImageDatasetGcnMask(Dataset):
	...
	def __getitem__(self, index):
		...
		img_path, pid, camid, cloid, msk_path, path_flag = self.dataset[index]
		# msk_path =
		img = read_image(img_path)
		msk = np.load(msk_path)  # <class 'tuple'>: (275, 85)
		# msk = torch.from_numpy(msk).unsqueeze(dim=0)  # torch.Size([1, 275, 85])
		msk = torch.from_numpy(msk).permute(2, 0, 1).unsqueeze(dim=0)  # torch.Size([1, 6, 275, 85])
		# msk = torch.nn.functional.interpolate(msk, size=(self.h, self.w), mode='bilinear', align_corners=True)
		img_concat = [img, msk]
		img_concat = self.transform(img_concat)  # [3, 256, 128]
		img = img_concat[0]
		msk = img_concat[1]
		# C, H, W = img.shape

		# torch.Size([1, 6, 256, 128]), min=8.4-06, max=0.9997

		return img, msk, pid, camid, cloid, path_flag
...
class ImageDataset_unsuper(Dataset):
	"""Image Person ReID Dataset"""
	...
	def __getitem__(self, index):
		...
		if self.transform is not None:
			img = self.transform(img)
		return img, img_path, pid, camid, path_flag
