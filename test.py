...
def concat_all_gather2(tensors, num_total_examples):
    '''
    Performs all_gather operation on the provided tensor list.
    '''
    outputs = []
...
@torch.no_grad()
def extract_img_feature_unsupervised(model, dataloader):
    features2, pids = [], []
    # features, pids, camids = features, pids.cuda(), camids.cuda()
    # path_flag = torch.tensor([])
    # path_flag = path_flag.cuda()
    for batch_idx, (imgs, _, batch_pids, _, _) in enumerate(dataloader):
        flip_imgs = torch.flip(imgs, [3])  # torch.Size([128, 3, 256, 128])
        batch_pids = batch_pids.cuda()
        # batch_path_flag = batch_path_flag.cuda()
        imgs, flip_imgs = imgs.cuda(), flip_imgs.cuda()
        batch_features = model(imgs)
        batch_features_flip = model(flip_imgs)
        batch_features0 = batch_features + batch_features_flip
        batch_features1 = F.normalize(batch_features0, p=2, dim=1)

        features2.append(batch_features1)
        pids.append(batch_pids)
        # camids = torch.cat((camids, batch_camids.cpu()), dim=0)
        # path_flag = torch.cat((path_flag, batch_path_flag), dim=0)
    features3 = torch.cat(features2, 0)  # torch.Size([8948, 4096])
    pids = torch.cat(pids, 0)
    return features3, pids


def extect_prcc(model, trainloarder, dataset):

    model.eval()
    # Extract features for total train set
    qsf, qs_pids = extract_img_feature_unsupervised(model, trainloarder)
    torch.cuda.empty_cache()

    qsf, qs_pids = concat_all_gather2([qsf, qs_pids], len(dataset.train))
    qsf, qs_pids = qsf.numpy(), qs_pids.numpy()

    torch.cuda.empty_cache()
    assert len(dataset.train) == 17896
    return qsf, qs_pids
