...
def search_raw_array_pytorch(res, xb, xq, k, D=None, I=None,
    ...
    nq, d = xq.size()  # nq:12936, d:4096
    if xq.is_contiguous():  # true
        xq_row_major = True
    ...
    nb, d2 = xb.size()  # nb:12936, d2:4096
    assert d2 == d
    if xb.is_contiguous():  # true
        xb_row_major = True
    ...
    if D is None:
        D = torch.empty(nq, k, device=xb.device, dtype=torch.float32)  # torch.Size([12936, 30])
    else:
        assert D.shape == (nq, k)
        assert D.device == xb.device

    if I is None:
        I = torch.empty(nq, k, device=xb.device, dtype=torch.int64)  # torch.Size([12936, 30])
    else:
        assert I.shape == (nq, k)
